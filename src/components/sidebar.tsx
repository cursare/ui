...
import { useMediaQuery } from "@cursare/ui/hooks/use-media-query";
import { cn } from "@cursare/ui/lib/utils";
import { Button } from "@cursare/ui/components/button";
import { Input } from "@cursare/ui/components/input";
import { ScrollArea } from "@cursare/ui/components/scroll-area";
import { Separator } from "@cursare/ui/components/separator";
import {
  Sheet,
  SheetDescription,
  SheetHeader,
  SheetPopup,
  SheetTitle,
} from "@cursare/ui/components/sheet";
import { Skeleton } from "@cursare/ui/components/skeleton";
import {
  Tooltip,
  TooltipPopup,
  TooltipTrigger,
} from "@cursare/ui/components/tooltip";
  ...
  React.useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent): void => {
      if (
        ...
        const providers = document.querySelectorAll<HTMLElement>(
          '[data-slot="sidebar-wrapper"]',
        );
        if (providers.item(providers.length - 1) !== wrapperRef.current) return;
...
export function Sidebar({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="flex h-full w-full flex-col bg-sidebar group-data-[variant=floating]/sidebar:rounded-lg group-data-[variant=floating]/sidebar:border group-data-[variant=floating]/sidebar:border-sidebar-border group-data-[variant=floating]/sidebar:shadow-sm/5"
          data-sidebar="sidebar"
...
export function SidebarTrigger({
  ...
  onClick,
  ...props
}: React.ComponentProps<typeof Button> & { label?: string }): React.ReactElement {
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "group-data-[collapsible=offcanvas]/sidebar:translate-x-0 hover:group-data-[collapsible=offcanvas]/sidebar:bg-sidebar group-data-[collapsible=offcanvas]/sidebar:after:left-full",
...
export function SidebarContent({
  ...
  return (
    <ScrollArea className="min-h-0 flex-1" fill scrollFade>
      <div
        className={cn(
          "flex h-full flex-col gap-2 group-data-[collapsible=icon]/sidebar:overflow-hidden",
          className,
        )}
